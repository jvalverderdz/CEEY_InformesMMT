...
/*********************************************************************************************
* Nombre archivo: 		RT_BoletinTrimestral_4.MovIngresoFormalidad_Estados
...
* Propósito:
	- Éste archivo genera y exporta los cálculos de movilidad en ingreso segmentados
	  por tipo de formalidad en el empleo-
*********************************************************************************************/
...
/* Variable de transición en el tipo de empleo. */
gen trans_empleo = .
...
*******************************************
* (4): Calcular variables de interés nacionales y por estado por cada tipo de formalidad
********************************************
...
quietly tab ent
scalar st = r(r)
mat output_matrix = J(st+1,28,0)

*Calculamos los 7 valores de movilidad de ingreso para cada tipo de empleo, y añadimos a la matriz output
...
foreach f of local formal{
	...
	qui sum ImInd if trans_empleo == `f' & ImInd > 0 & ImInd !=. [fw = factor]
	replace TOTcmovAsc`f' = r(sum_w) if trans_empleo == `f'
	
	qui sum ImInd if trans_empleo == `f' & ImInd < 0 [fw = factor]
	replace TOTcmovDes`f' = r(sum_w) if trans_empleo == `f'
	
	qui total hola if trans_empleo ==`f' & ImInd==. [fw=factor]
	replace TOTcmovNul`f' = e(N) if trans_empleo == `f'
	
	gen TOTtotalobs`f' = .
	replace TOTtotalobs`f' = TOTcmovAsc`f' + TOTcmovDes`f' + TOTcmovNul`f' if trans_empleo == `f'
	gen TOTpctmovAsc`f' = .
	replace TOTpctmovAsc`f' = TOTcmovAsc`f' / TOTtotalobs`f' if trans_empleo == `f'
	gen TOTpctmovDes`f' = .
	replace TOTpctmovDes`f' = TOTcmovDes`f' / TOTtotalobs`f' if trans_empleo == `f'
	gen TOTpctmovNul`f' = .
	replace TOTpctmovNul`f' = TOTcmovNul`f' / TOTtotalobs`f' if trans_empleo == `f'
...
putexcel set "$oldroot\Entidades Federativas\RT_BoletinTrimestral_Estados.xlsx", sheet("3.1. Mov. INGRESO Formalidades") modify
...
foreach i of local estado {
	...
	foreach f of local formal{
		...
		qui sum ImInd if trans_empleo == `f' & ImInd > 0 & ImInd !=. [fw = factor]
		replace ENTcmovAsc`f' = r(sum_w) if trans_empleo == `f'
		
		qui sum ImInd if trans_empleo == `f' & ImInd < 0 [fw = factor]
		replace ENTcmovDes`f' = r(sum_w) if trans_empleo == `f'
		
		qui total hola if trans_empleo ==`f' & ImInd==. [fw=factor]
		replace ENTcmovNul`f' = e(N) if trans_empleo == `f'
		
		gen ENTtotalobs`f' = .
		replace ENTtotalobs`f' = ENTcmovAsc`f' + ENTcmovDes`f' + ENTcmovNul`f' if trans_empleo == `f'
		gen ENTpctmovAsc`f' = .
		replace ENTpctmovAsc`f' = ENTcmovAsc`f' / ENTtotalobs`f' if trans_empleo == `f'
		gen ENTpctmovDes`f' = .
		replace ENTpctmovDes`f' = ENTcmovDes`f' / ENTtotalobs`f' if trans_empleo == `f'
		gen ENTpctmovNul`f' = .
		replace ENTpctmovNul`f' = ENTcmovNul`f' / ENTtotalobs`f' if trans_empleo == `f'
...
putexcel J1=("Trabajadores que obtuvieron empleo formal")
...
putexcel Q1=("Trabajadores que perdieron empleo formal")
putexcel Q2=("Total de observaciones")
putexcel R2=("Porcentaje de ascenso")
putexcel S2=("Promedio de ascenso")
putexcel T2=("Porcentaje de descenso")
putexcel U2=("Promedio de descenso")
putexcel V2=("Porcentaje sin cambio")
putexcel W2=("Promedio general")

putexcel X1=("Trabajadores que nunca tuvieron empleo formal")
putexcel X2=("Total de observaciones")
putexcel Y2=("Porcentaje de ascenso")
putexcel Z2=("Promedio de ascenso")
putexcel AA2=("Porcentaje de descenso")
putexcel AB2=("Promedio de descenso")
putexcel AC2=("Porcentaje sin cambio")
putexcel AD2=("Promedio general")
